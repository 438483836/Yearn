...
//    @Value("${spring.web.accessKeySecret}")
    ...
    public Map<String, Object> sendSms(List<String> numberList, String signName, String templateCode, Map<String, String> templateMap){
        ...
        try {
            StringBuffer phoneNumbers = new StringBuffer(numberList.get(0));
            for (String num : numberList) {
                phoneNumbers = phoneNumbers.append(","+num);
            }
...
//            request.setOutId("yourOutId12");
            //请求失败这里会抛ClientException异常
            System.out.println("短信发送接口入参 request：" + JSONUtils.toString(request));
            SendSmsResponse sendSmsResponse = acsClient.getAcsResponse(request);
            System.out.println("短信发送接口响应结果 =======> sendSmsResponse:" + JSONUtils.toString(sendSmsResponse));
            if(sendSmsResponse.getCode() != null && sendSmsResponse.getCode().equals("OK")) {
                //请求成功
            }
            result.put("code", sendSmsResponse.getCode());
            result.put("result", sendSmsResponse);
            return result;
        } catch (Exception e){
            e.printStackTrace();
            return null;
